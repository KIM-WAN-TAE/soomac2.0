...
# 설정
# =========================
WEIGHTS    = "src/zeus_vision/jinjja_jjinmak_injung.pt"
DEVICE     = "0"
...
AREA_THRESHOLD_MM2 = 1800.0   # ★ 추가: 면적 임계(mm^2). 이보다 작으면 후보 제외
...
def area_mm2_from_inliers(inliers_xyz, plane_origin, x_axis, y_axis):
    ...
    rel = P - o
    X = (rel @ xa).reshape(-1)   # [m]
    Y = (rel @ ya).reshape(-1)   # [m]
    pts2 = np.stack([X, Y], axis=1)

    # 컨벡스 헐(2D)
    hull = cv2.convexHull(pts2.astype(np.float32), returnPoints=True)
    hull = hull.reshape(-1, 2)  # [m]

    area_m2 = _polygon_area_2d(hull)  # [m^2]
    return float(area_m2 * 1e6)       # [mm^2]
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            if mode == 'block':
    # 1) 후보 전처리: 가로면 후보만 남기고, 폭/면적 임계 동시 만족(block 게이트)
                valid = []
                for c in candidates:
                    if c.get('origin') is None:
                        continue

                    # 면 분류(기존): length_area 제외
                    face = c.get('face', 'uncertain')
                    if face != 'width_area':
                        continue

                    # 폭(mm): 기존 detect_area 결과가 c['width_mm']로 저장되어 있음
                    width_mm = c.get('width_mm', None)

                    # 면적(mm^2): inliers 기반 계산
                    inliers = c.get('inliers', None)
                    x_axis, y_axis, z_axis = c.get('axes', (None, None, None))
                    plane_model = c.get('plane_model', None)
                    origin_m = c.get('origin', None)

                    area_mm2 = area_mm2_from_inliers(inliers, origin_m, x_axis, y_axis)

                    # block 게이트: 폭 임계 + 면적 임계 모두 통과해야 함
                ...
                # 2) 선택 규칙: (기존 block0) 최소 z 층(±SAME_LAYER_EPS_M) → (x,y) 최소 → z 더 작음 → |x|+|y| 최소
                chosen = None
                if valid:
                    ...
                    def _xy_norm(x, y): return math.hypot(x, y)
                    def key_block(v):
                        ox, oy, oz = map(float, v['origin'])
                        dxy = _xy_norm(ox, oy)
                        l1  = abs(ox) + abs(oy)
                ...
                if chosen is not None:
                    ...
                    cx, cy = chosen['cx'], chosen['cy']  # 이미지상 중심(픽셀)
