...
MIN_LIMITS = np.array([np.deg2rad(-160), np.deg2rad(-115), np.deg2rad(-115), np.deg2rad(-100)])
MAX_LIMITS = np.array([np.deg2rad( 100), np.deg2rad( 115), np.deg2rad( 115), np.deg2rad( 100)])
...
def solve_ik_position_only(target_pos, initial_q_active=None,
    ...
    # Calculate initial guess for th_z
    th_z_initial = np.arctan2(y, x)
    th_z_initial = np.clip(th_z_initial, MIN_LIMITS[0], MAX_LIMITS[0])
    ...
    if not result.success:
        # Retry with different initial guess if failed
        x0 = [np.pi/4, -np.pi/4]
        result = minimize(objective, x0, bounds=bounds, method='L-BFGS-B')
    ...
    # Apply bounds using minimize for all angles
