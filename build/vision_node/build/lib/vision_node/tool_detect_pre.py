...
# obb_handle_tip_from_mask 함수를 아래 코드로 교체하세요.
...
def obb_handle_tip_from_mask(mask, out_size_hw, end_band_ratio=0.18, min_pts=30):
    ...
    1. 마스크의 컨벡스 헐로 minAreaRect를 찾아 객체의 강건한 방향을 얻는다.
    2. 해당 사각형의 장축을 주축(major axis)으로 삼는다.
    3. 주축 기준으로 양 끝 밴드를 잘라 손잡이/날을 결정한다.
    4. 최종 방향은 (손잡이 중심 -> 날 중심)으로 정의한다.
    """
    H, W = out_size_hw
    if mask.dtype != np.uint8:
    ...
    # 마스크의 모든 점(pts)과 중심(mean) 계산 (기존 코드와 동일)
