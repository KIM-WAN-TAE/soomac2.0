# -*- coding: utf-8 -*-
"""
[최종 솔루션 v6.4 — timer_callback 제거·메인루프 이관 + 시간필터링 제거 + 안정화 퍼블리시 게이트]
- timer_callback 로직을 main()의 while rclpy.ok(): 루프로 이동
- 안정화(time filtering) 제거: 히스토리/평균 없이 단일 프레임 기준으로 RPY 산출
- 실제 블록 크기 기반(minAreaRect 스냅) 중심 보정 유지
- YOLO 세그 노이즈 제거(2D CC, plane inlier, 3D DBSCAN) 유지
- Matplotlib 백엔드 자동 폴백(TkAgg/Qt5Agg/Agg)
- ROS 2 Float32MultiArray(3x3) 퍼블리시(최종 제약 RPY 기반 회전행렬)
- FIX: set_zlim 안전가드
- NEW: 쿼터니언 각거리 Δθ 기반 "자세 변화 거의 없음" 조건에서만 퍼블리시(연속 N프레임+쿨다운)
"""
import os
...
# 설정값 (튜닝 포인트 ★)
# =========================
WEIGHTS    = "/home/wt/zeus_wt_ws/src/vision_node/vision_node/green_purple_blue.pt"
DEVICE     = "0"        # '0' 또는 0
...
# ---- 안정화 퍼블리시 게이트 설정 (NEW) ----
STAB_ANGLE_DEG = 0.5         # Δθ 임계각(도) — 이 값보다 작으면 "거의 변화 없음"으로 판단
...
def refine_center_minarearect_with_size(mask_bin, depth_frame, intr, depth_scale, L_m, W_m):
    """
    mask_bin(plane_inlier 권장)에서 minAreaRect → 실제 크기 스냅 → 교집합 3D 평균으로 중심 보정
    ...
    w_exp1, h_exp1 = _expected_px_from_metric(L_m, W_m, z_med, fx, fy)  # (긴, 짧)
    w_exp2, h_exp2 = _expected_px_from_metric(W_m, L_m, z_med, fx, fy)  # 가설 전환
        ...
        while rclpy.ok():
                ...
                for i in idxs:
                    ...
                    if USE_DBSCAN_3D and inliers is not None and len(inliers) >= 50:
                        ...
                        if main is not None and len(main) >= 50:
                            origin_m = main.mean(axis=0).astype(np.float32)
                            cov = np.cov(main.T)
                            evals, evecs = np.linalg.eigh(cov)
                            axes = evecs.T[np.argsort(evals)]
                            x_axis = axes[2]
            ...
            if chosen is not None:
                ...
                if all(v is not None for v in [x_axis, y_axis, z_axis, origin_m]):
                    ...
                    # 2) (시간 필터링 제거) 단일 프레임 기준 쿼터니언/이울러
                    ...
                    # ---------- NEW(안정화) 퍼블리시 게이트 ----------
                    # 대상 변경/점프 감지: 중심 픽셀 이동량이 크면 안정화 리셋
                    ...
                    should_publish = (node.stable_count >= STAB_MIN_FRAMES) and (node.cooldown == 0)
                    ...
                    if should_publish:
                        ...
                        msg.data = final_rot_matrix.flatten().astype(np.float32).tolist()
                        ...
                        node.stable_count = 0  # 동일 포즈 반복 퍼블리시 방지

                    # 2D 텍스트(상태 표시 보강: Δθ/카운트/쿨다운)
                    cv2.putText(
                        overlay,
