...
class MainControlNode(Node):
    def __init__(self):
        ...
        self.create_subscription(Bool, '/zeus/state/no_candidate', self.no_candidate, 10)
    ...
    # True -> 없음
    def no_candidate(self, msg : Bool):
        if msg.data:
            with self.lock:
                if self.current_flag == 'waiting':
                    self.next_step = 'emer_1'
                    self.current_flag = 'done'
    ...
    def loop(self):
        with self.lock:
            if self.start_ping is None:
                self.start_ping = input("시작 하려면 start를 입력하세요 : ")
                self.handler = Block()
