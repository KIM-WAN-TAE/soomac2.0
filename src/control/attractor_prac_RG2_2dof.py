...
envs = []
franka_handles = []
franka_hand = "ur_grip_site"

# Attractor setup
attractor_handles = []
attractor_properties = gymapi.AttractorProperties()
attractor_properties.stiffness = 5e5
attractor_properties.damping = 9e3
...
sphere_rot = gymapi.Quat.from_euler_zyx(0.1 * math.pi, 0, 0)
...
franka_dof_props['stiffness'].fill(10.0)
...
franka_dof_props["driveMode"][5:] = gymapi.DOF_MODE_POS
franka_dof_props['stiffness'][5:] = 1000
franka_dof_props['damping'][5:] = 1.0
...
def update_franka(t):
    ...
    for i in range(num_envs):
        ...
        pose.p.x = 0.3 + 0.1 * math.sin(1.5 * t - math.pi * float(i) / num_envs)
        pose.p.y = 0.3 + 0.1 * math.cos(2.5 * t - math.pi * float(i) / num_envs)
        pose.p.z = 0.1 * math.cos(1.5 * t - math.pi * float(i) / num_envs)
