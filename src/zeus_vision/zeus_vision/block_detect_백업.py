...
class BlockPosePublisher(Node):
    def __init__(self):
        ...
        # <<< ADD: '디텍 불가' 상태 통지 퍼블리셔 및 타이머 상태 변수 >>>
        self.no_cand_pub = self.create_publisher(Bool, '/zeus/state/no_candidate', 10)
...
def main(args=None):
    ...
    try:
        while rclpy.ok():
            ...
            # (ADD) '디텍 불가' 3초 타이머 로직
            # - 요청 전환/종료 시 즉시 리셋
            # - YOLO를 실제 실행한 프레임(res is not None)에서 chosen이 None일 때만 누적
            # - 3초 연속 충족 시 1회 발행 후 즉시 리셋
            # ==========================
