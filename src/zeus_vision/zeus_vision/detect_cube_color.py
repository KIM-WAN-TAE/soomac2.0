...
WIDTH_THRESHOLD_MM = 24.0       # 폭(mm) 기준
AREA_THRESHOLD_MM2 = 1600.0     # 면적 임계(mm^2)
PRIORITY_CLASSES = ['red', 'green', 'pink', 'purple', 'blue', 'yellow']
...
def refine_center_minarearect_with_size(mask_bin, depth_frame, intr, depth_scale, L_m, W_m,
    ...
    X = (cx - intr.ppx) * z_med / intr.fx
    Y = (cy - intr.ppy) * z_med / intr.fy
    refined_origin = np.array([X, Y, z_med], dtype=np.float32)
...
def area_mm2_from_inliers(inliers_xyz, plane_origin, x_axis, y_axis):
    ...
    hull = cv2.convexHull(pts2.astype(np.float32), returnPoints=True)
    hull = hull.reshape(-1, 2)
    area_m2 = _polygon_area_2d(hull)
    ...
    try:
        while rclpy.ok():
            ...
            if res and res[0].boxes is not None and len(res[0].boxes) > 0:
                ...
                # ---- (추가) 우선순위 클래스 한 종만 선택 ----
                # names가 list/dict 모두 동작하도록 정규화
                if isinstance(names, dict):
                    name_map = names  # {class_id: class_name}
                else:
                    name_map = {i: n for i, n in enumerate(names)} if names is not None else {}

                selected_cid = None
                idxs = None
                for cname in PRIORITY_CLASSES:
                    # 현재 우선순위 이름을 가진 class_id 찾기
                    cid_list = [k for k, v in name_map.items() if v == cname]
                    if not cid_list:
                        continue
                    cid = cid_list[0]
                    # 이 클래스의 고신뢰 디텍션(>= CONF_PUB)만 추립니다.
                    idxs_c = np.where((confs >= CONF_PUB) & (clss == cid))[0]
                    if idxs_c.size > 0:
                        selected_cid = cid
                        idxs = idxs_c
                        break

                # 어떤 우선순위 클래스도 없다면, 이 프레임은 후처리 스킵
                ...
                for i in idxs:
                    ...
                    # ===== 폭(mm) / 면적(mm^2) UI 표시 =====
                    # 면적(mm^2) 계산(평면 내 inliers 기준)
                    area_mm2_ui = area_mm2_from_inliers(inliers, origin_m, x_axis, y_axis)
                    if origin_m is not None and np.isfinite(origin_m[2]):
                        zc = float(origin_m[2])
                        # 기존 폭/면적 라인(y_w=+18, y_a=+36)을 고려해 그 아래(+54)에 배치
                        y_z = min(COLOR_H - 6, y2 + 54)
                        txt_z = f"z={zc:.3f} m"
                        cv2.putText(overlay, txt_z, (x1, y_z), FONT, 0.55, (255,255,255), 3, cv2.LINE_AA)
                        cv2.putText(overlay, txt_z, (x1, y_z), FONT, 0.55, (0,0,0), 1, cv2.LINE_AA)

                    # 표시 위치: 바운딩박스 왼쪽-아래
                    y_w = min(COLOR_H - 6, y2 + 18)
                    y_a = min(COLOR_H - 6, y2 + 36)

                    # 폭 표기
                    if (min_width_mm is not None) and np.isfinite(min_width_mm):
                        txt_w = f"w={min_width_mm:.1f} mm"
                        # cv2.putText(overlay, txt_w, (x1, y_w), FONT, 0.55, (255,255,255), 3, cv2.LINE_AA)
                        # cv2.putText(overlay, txt_w, (x1, y_w), FONT, 0.55, (0,0,0), 1, cv2.LINE_AA)

                    # 면적 표기
                    if (area_mm2_ui is not None) and np.isfinite(area_mm2_ui):
                        txt_a = f"A={area_mm2_ui:.0f} mm^2"
                        # cv2.putText(overlay, txt_a, (x1, y_a), FONT, 0.55, (255,255,255), 3, cv2.LINE_AA)
                        # cv2.putText(overlay, txt_a, (x1, y_a), FONT, 0.55, (0,0,0), 1, cv2.LINE_AA)
            ...
            if mode == 'block':
                ...
                for c in candidates:
                    if c.get('origin') is None:
                        print('1')
                        continue
                    if c.get('face', 'uncertain') != 'width_area':
                        print('2')
                        continue

                    width_mm = c.get('width_mm', None)
                    ...
                    if not block_pass_filter(width_mm=width_mm,
                                            area_mm2=area_mm2,
                                            width_thr_mm=WIDTH_THRESHOLD_MM,
                                            area_thr_mm2=AREA_THRESHOLD_MM2):
                        continue
                    c['area_mm2'] = area_mm2
                    valid.append(c)
                # print(valid)
                # print(time.time() - pre_block)
                # 2) 선택 규칙
                chosen = None
                if valid:
                ...
                if chosen is not None:
                    ...
                    if node.detect_signal != "":
                        ...
                        node.stable_count = 0
                        node.detect_signal = ""  # 1회 발행 후 닫기
                        # print(final_px)

                    # OSD
                    if final_px is not None:
                        info1 = f"CHOSEN w={width_mm:.2f} mm" if (width_mm is not None) else "CHOSEN"
                        cv2.putText(overlay, info1,
                                    (max(0, final_px[0]-70), min(COLOR_H-8, final_px[1]+18)),
                                    FONT, 0.55, (255,255,255), 3, cv2.LINE_AA)
                        cv2.putText(overlay, info1,
                                    (max(0, final_px[0]-70), min(COLOR_H-8, final_px[1]+18)),
                                    FONT, 0.55, (0,0,255), 1, cv2.LINE_AA)

                    if area_mm2 is not None and final_px is not None:
                        info2 = f"area={area_mm2:.0f} mm^2"
                        cv2.putText(overlay, info2,
                                    (max(0, final_px[0]-70), min(COLOR_H-8, final_px[1]+36)),
                                    FONT, 0.55, (255,255,255), 3, cv2.LINE_AA)
                        cv2.putText(overlay, info2,
                                    (max(0, final_px[0]-70), min(COLOR_H-8, final_px[1]+36)),
                    ...
                    cv2.putText(
                        overlay,
                        f"BLOCK  R:{final_rpy[0]:.1f} P:{final_rpy[1]:.1f} Y:{final_rpy[2]:.1f} "
                        f"| dθ:{(dtheta if np.isfinite(dtheta) else 999):.3f}° | jump:{center_jump:.1f}px | "
                        f"stab:{node.stable_count}/{STAB_MIN_FRAMES} | cd:{node.cooldown}",
                        (max(0, cx-260), max(20, cy-20)),
