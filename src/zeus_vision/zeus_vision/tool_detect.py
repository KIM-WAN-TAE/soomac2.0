...
WEIGHTS = "/home/pc/soomac_ws/src/zeus_vision/tool_best.pt"  # seg/det 둘 다 허용
DEVICE = "0"
CONF_TH = 0.28
IOU_TH  = 0.45
IMG_SIZE = 960
...
SHOW_BAND_PREVIEW = True   # ★ True면 choose_handle_tip 이전 '후보군'을 시각화
            ...
            if self.target_tool == "M3" and self.detection_active:
                ...
                if ids is not None:
                    ...
                    for i, mid in enumerate(ids.flatten()):
                        if mid != int(0):
                            print("Detected ARUCO ID not 0, continue")
                            continue
                        print("Detected ARUCO ID 0")
                        # 크기 일관성 필터
