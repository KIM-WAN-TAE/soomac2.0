...
AREA_THRESHOLD_MM2 = 1700.0     # 면적 임계(mm^2)
...
class BlockPosePublisher(Node):
    def __init__(self):
        ...
        # # --- 사용자 카메라 보정행렬(K)과 왜곡계수(D) ---
        # self.K = np.array([[2.59078906e+03, 0.0,            3.17046217e+02],
        #                 [0.0,            2.64501049e+03, 3.13012151e+01],
        #                 [0.0,            0.0,            1.0]], dtype=np.float32)
        # self.D = np.array([1.2966898, 2.07518645, -0.09900567, 0.03112062, 0.67733035], dtype=np.float32)

        # # 컬러·깊이 해상도에 맞춰 undistort 맵 준비
        # self.newK, _ = cv2.getOptimalNewCameraMatrix(self.K, self.D, (COLOR_W, COLOR_H), 0)
        # self.map1, self.map2 = cv2.initUndistortRectifyMap(
        #     self.K, self.D, None, self.newK, (COLOR_W, COLOR_H), cv2.CV_32FC1
        # )

        # # 투영/역투영용 단순 intrinsics 객체로 교체
        # class SimpleIntr:
        ...
        # self.intr = SimpleIntr(
        #     fx=self.newK[0,0], fy=self.newK[1,1],
        #     ppx=self.newK[0,2], ppy=self.newK[1,2]
        # )
