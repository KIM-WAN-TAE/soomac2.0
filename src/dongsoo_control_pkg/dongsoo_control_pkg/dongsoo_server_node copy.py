...
class DongsooServer(Node):
    ...
    def joint_deg_callback(self, msg : Float32MultiArray):
        data = list(msg.data)
        present_q = [0.0, 0.0, 0.0, 0.0]
        ...
        with self.lock:
            for idx, q in enumerate(msg.data):
                present_q[idx] = float(q)
            self.present_j = present_q[:4]
            self.present_wrist = present_q[4]
