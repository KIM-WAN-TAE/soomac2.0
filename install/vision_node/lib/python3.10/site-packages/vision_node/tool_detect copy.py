...
SHOW_BAND_PREVIEW = True   # ★ 추가: True면 choose_handle_tip 이전 '후보군'을 시각화
...
def obb_handle_tip_from_mask(mask, out_size_hw, end_band_ratio=0.18, min_pts=30):
    ...
    - ★ 추가: choose_handle_tip 이전 단계의 '밴드 후보군' 시각화를 위해 debug dict 반환
    반환:
      ...
      debug(dict)  # {'band_head_img','band_tail_img','u_major','v_minor','mean','t_min','t_max','head_ctr','tail_ctr'}
    """
    H, W = out_size_hw
    if mask.dtype != np.uint8:
    ...
    # 주축으로 양 끝 밴드 (★ 여기까지가 "알고리즘 전" 후보군 지정)
    ...
    mask_head = proj_u <= (t_min + band)   # 앞쪽 (HEAD 후보)
    mask_tail = proj_u >= (t_max - band)   # 뒤쪽 (TAIL 후보)

    # --- ★ 프리뷰용 이미지 마스크 생성 (H×W) ---
    band_head_img = np.zeros((H, W), dtype=np.uint8)
    band_tail_img = np.zeros((H, W), dtype=np.uint8)
    if np.any(mask_head):
        pts_h = np.round(pts[mask_head]).astype(np.int32)
        pts_h[:,0] = np.clip(pts_h[:,0], 0, W-1)
        pts_h[:,1] = np.clip(pts_h[:,1], 0, H-1)
        band_head_img[pts_h[:,1], pts_h[:,0]] = 1
        head_ctr = pts_h.mean(axis=0).astype(np.int32)
    else:
        head_ctr = None

    if np.any(mask_tail):
        pts_t = np.round(pts[mask_tail]).astype(np.int32)
        pts_t[:,0] = np.clip(pts_t[:,0], 0, W-1)
        pts_t[:,1] = np.clip(pts_t[:,1], 0, H-1)
        band_tail_img[pts_t[:,1], pts_t[:,0]] = 1
        tail_ctr = pts_t.mean(axis=0).astype(np.int32)
    else:
        tail_ctr = None

    debug = dict(
        band_head_img=band_head_img,
        band_tail_img=band_tail_img,
        u_major=u_major.copy(),
        v_minor=v_minor.copy(),
        mean=mean.copy(),
        t_min=float(t_min),
        t_max=float(t_max),
        head_ctr=head_ctr,
    ...
    len_x = (maxs[0] - mins[0])
    len_y = (maxs[1] - mins[1])
    return corners, (len_x, len_y), angle_deg, center, handle_ctr, tip_ctr, (h_end_a, h_end_b, t_end_a, t_end_b), decide_conf, debug
            ...
            if results and len(results) > 0:
                ...
                for i in range(N):
                    ...
                    if masks_np is not None and i < masks_np.shape[0]:
                        ...
                        # ★ 후보군 프리뷰: choose_handle_tip 이전 '밴드'를 색으로 보여줌
