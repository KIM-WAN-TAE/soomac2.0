...
WEIGHTS = "/home/temp_id/ros2_ws/src/vision_node/vision_node/best.pt"  # seg/det 둘 다 허용
DEVICE = "0"
CONF_TH = 0.28
IOU_TH  = 0.45
IMG_SIZE = 960
...
TARGET_LABELS = {"nipper", "vernier_calipers", "sazqwaszwased", "wire_stripper"}
SHOW_WINDOW = True
...
def obb_handle_tip_from_mask(mask, out_size_hw, end_band_ratio=0.18, min_pts=30):
    """
    세그 마스크로 OBB 계산(이미지 좌표계):
    - x축을 '손잡이 중앙 → 날 중앙' 방향으로 강제 정렬.
    - 각 끝단 밴드에서 v축 최상/최하 끝점 2개 → 그 중점을 중앙점으로 사용.
    반환:
    ...
    corners: OBB 4꼭짓점(시계 방향).
    len_x, len_y: OBB의 x축/ y축 길이(픽셀 단위).
    angle_deg: 화면 기준 b=(0,-1)(위쪽)을 기준으로 x축(손잡이→날)의 시계방향 - 각도(도).
    center: OBB 중심(픽셀).
    
    handle_ctr, tip_ctr: 손잡이/날 중앙점(픽셀).
    
    h_end_a, h_end_b, t_end_a, t_end_b: 손잡이/날 밴드에서 v축 최상·최하 끝점 2개(픽셀).
    decide_conf: 손잡이/날 판정 신뢰도(폭/두께/뾰족도 기반).
    """
    H, W = out_size_hw
    if mask.dtype != np.uint8:
    ...
    u_major = vecs[:, order[0]]; u_major /= (np.linalg.norm(u_major) + 1e-9)
    v_minor = vecs[:, order[1]]; v_minor /= (np.linalg.norm(v_minor) + 1e-9)
    ...
    def band_endpoints_and_center(band_mask):
        ...
        ctr_b = centered[band_mask]
        v_b   = ctr_b @ v_minor
        i_min = int(np.argmin(v_b)); i_max = int(np.argmax(v_b))
        p_min = pts_b[i_min]; p_max = pts_b[i_max]
...
class VisionNode(Node):
    ...
    def vision_callback(self):
        try:
            ...
            # color 프레임이 없으면 표시할 수 없으므로 반환
            if not color_frame:
                return

            # 깊이 필터 체인 (depth 미존재시 스킵)
            if depth_frame:
            ...
            if results and len(results) > 0:
                ...
                for i in range(N):
                    ...
                    # 3D 중심 (bbox/OBB의 center pick: mask가 있으면 center_px, 없으면 bbox 중앙)
                    ...
                    # # 손잡이/날 3D (가능하면 추정)
                    # handle_3d = tip_3d = None
                    # if handle_ctr is not None:
                    #     dh = median_depth_meters_from_center(depth_frame, int(handle_ctr[0]), int(handle_ctr[1]), k=DEPTH_KERNEL, depth_scale=self.depth_scale) if depth_frame is not None else None
                    #     if dh is None: dh = depth_m
                    #     if dh is not None:
                    #         handle_3d = self.pixel_to_3d_point(handle_ctr[0], handle_ctr[1], dh)
                    # if tip_ctr is not None:
                    #     dtv = median_depth_meters_from_center(depth_frame, int(tip_ctr[0]), int(tip_ctr[1]), k=DEPTH_KERNEL, depth_scale=self.depth_scale) if depth_frame is not None else None
                    #     if dtv is None: dtv = depth_m
                    #     if dtv is not None:
                    #         tip_3d = self.pixel_to_3d_point(tip_ctr[0], tip_ctr[1], dtv)
            ...
            if found_payload is not None:
                ...
                # 1회 완료 후 비활성화 (tool_detect와 동일 정책)
                self.detection_active = False
                self.target_tool = None
